...
class AliasSetTracker {
 public:
  ...
  bool isAlias(const Value* a, const Value* b) const {
    ...
    for (auto aElement : aSets) {
      for (auto bElement : bSets) {
        if (find(aElement) == find(bElement)) {
          return true;
        }
      }
    }
  ...
  bool makeAlias(const Value* a, const Value* b) {
    ...
    for (const auto& set : map_.at(b)) {
      if (map_.count(a) && !map_.at(a).count(set)) {
        changed = true;
      }
 ...
 private:
  ...
  // Mapping values to their corresponding alias entry
  std::unordered_map<const Value*, std::unordered_set<Element*>> map_;
  // All values that are wildcards (e.g. they may alias anything)
  ...
  // Special set representing wildcard values. Nullptr means empty (i.e. there
  // are no wildcards in the graph)
  Element* wildcardSet_ = nullptr;
};

// We want to partition all values into distinct sets, where membership in a set
// denotes an aliasing relationship.
//
//
...
c10::optional<const Node*> AliasDb::getLastWildcard() const {
  const auto& wildcardNodes = setTracker_->getWildcardNodes();
  auto it = std::max_element(
      wildcardNodes.cbegin(),
      wildcardNodes.cend(),
      [this](const Node* a, const Node* b) { return isBeforeSameGraph(a, b); });
  if (it != wildcardNodes.end()) {
    return *it;
  } else {
    return c10::nullopt;
  }
